...
# two ways to assign a variable:
a = 1
# or
a <- 1

is.integer(a) # function to check if "a" is an integer (mode)
a <- as.integer(a) # formatting the content of variable "a" to the type integer (class)
is.integer(a) # check again, to confirm our formation process

a = 1L # direct assignment of a variable with an integer value
is.integer(a)
...
b = 2.3
is.double(b)
# is.numeric() works here too, but technically those two are different functions 
...
list_a[1:2] #positions 1 to 2

list_a[-1] #all positions without first
...
# You can put nearly everything into a list and even mix them together. 
...
dir.create("./Seminar2023") # creates a folder, location and name of the folder is 
...
data01 <- read.csv2("../nextcloud/1st Session/Jonas20230605.csv")
...
files <- list.files("./SDM-course-main/2023/1st Session/Data01/", recursive = F, full.names = T) # we take the result 
...
write.csv2(data, file ="./SDM-course-main/2023/1st Session/Data01/iris_all.csv")
